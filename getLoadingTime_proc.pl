...
#
	my $factorStart = 2;				# under 2, 2~3, 3~4....
	my $factorCount = 5;				# how many columns...
	my $dpTimeToFile = 1;				# option : saving text file for Displayed time.
	my $assumeResumeTimeAs = 100;		# assume the time for resume any activities.
	
	my $opIgnoreProcName = "com.nhn.android.search";	# process to ignore
	...
	while (my $line = <$fh> )
	{
		if ($job == 2)	# parsing event log
		{
			if ($line =~ /^(\d+-\d+\s\d+:\d+:\d+)\.\d+\sI\/activity_launch_time.*:\s\[\d+,(\S+),(\S+),(\S+)\]/ )
			{
				...
				#my $proc = getProcName($2);
				my $proc = $1 if ($AppName =~ /(\S+)\/\S+/);	# get process name without activity name.
				my $durSec = ($dur1 > $dur2)? $dur2:$dur1;
...
=cut			
		...
		{		
			...
			{	# Launched.
				...
				#print "000 - $1, $2\n";
				$justLaunched = $1 if ( $2 =~ /(\S+)\/\S+/);	# get process name without activity name.
				pushToHashResume( $justLaunched, 0);
